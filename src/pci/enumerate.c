...
void pci_enumerate_fn(uint64_t device_address, uint64_t bus, uint8_t device, uint8_t function) {
  ...
  if (!pci_fn->device_id || pci_fn->device_id == 0xFFFF) return;

  printf("%x:%x.%x -> %s / %s / %s / %s\r\n",
	bus, device, function,
...
void pci_enumerate_bus(uint64_t base_address, uint64_t bus) {
  ...
  if (!pci_bus->device_id || pci_bus->device_id == 0xFFFF) return;
