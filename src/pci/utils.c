...
static uintptr_t get_device_addr(mcfg_entry_t *entry, uint8_t bus, uint8_t device, uint8_t function) {
  return (
    entry->base_address + PHYS_MEM_OFFSET
...
int pci_get_bar(uint16_t segment, uint8_t bus, uint8_t device, uint8_t function, int bar, struct pci_bar *bar_info) {
  ...
  uint32_t bar_value = pci_cfg_read_dword(segment, bus, device, function, bar_offset);
  ...
  if (kind == PCI_BAR_KIND_MMIO) {
    ...
    if (bitness == PCI_BAR_BITNESS_16) return 1; // Forbidden
  }
  ...
  if (bar_size == 0) return 1; // Non-existent bar
  ...
  if (bitness == PCI_BAR_BITNESS_32) {
    ...
    bar_info->base = 
      ((uint64_t) pci_cfg_read_dword(segment, bus, device, function, bar_offset + 4) << 32)
      | (bar_value & ~0b1111);
  }
